    ...
    for file in files:
        ...
        while vcap.isOpened():
            print(f"Start with file: {file},\
                ...
                width/height: {int(vcap.get(cv2.CAP_PROP_FRAME_WIDTH))/int(vcap.get(cv2.CAP_PROP_FRAME_HEIGHT))},\
                frames count: f{int(vcap.get(cv2.CAP_PROP_FRAME_COUNT))}")
            break
